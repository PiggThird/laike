      ...
      <div class="cart-body" id="cartBody">
        ...
        <div class="cart-body-table">
          <div class="item" v-for="course_info in cart.select_course_list">
            <div class="item-2">
              <router-link :to="`/project/${course_info.id}`" class="img-box l"><img :src="course_info.course_cover">
              </router-link>
            ...
            <div class="item-3">
              <div class="price">
                <p class="discount-price" v-if="course_info.discount.price>=0">
                  <em>￥</em><span>{{ parseFloat(course_info.discount.price).toFixed(2) }}</span></p>
                <p :class="{'original-price': course_info.discount.price>=0}">
                  <em>￥</em><span>{{ parseFloat(course_info.price).toFixed(2) }}</span></p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="row-bottom">
            <div class="row">
              <div class="goods-total-price-box">
                <p class="r rw price-num"><em>￥</em><span>{{ parseFloat(cart.total_price).toFixed(2)}}</span></p>
                <p class="r price-text"><span>共<span>{{ cart.select_course_list?.length }}</span>件商品，</span>商品总金额：
                </p>
              </div>
            </div>
            <div class="coupons-discount-box">
              <p class="r rw price-num">-<em>￥</em><span>{{ parseFloat(order.discount_price).toFixed(2) }}</span></p>
...
.cart-header {
  ...
  background: url("@/assets/cart-header-bg.jpeg") repeat-x;
  background-size: 38%;
}
